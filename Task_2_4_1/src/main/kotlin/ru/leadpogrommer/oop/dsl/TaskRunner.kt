...
class TaskRunner(val config: Config) {
    ...
    private val repoBasePath =
        Paths.get(Paths.get(".").toAbsolutePath().toString(), "student_repos").toAbsolutePath().toString()
    ...
    private suspend fun latestCommitDateForDir(repo: Git, path: Path): LocalDate? = withContext(Dispatchers.IO) {
        ...
        return@withContext LocalDate.ofInstant(
            Instant.ofEpochSecond(latestCommitTime.toLong()),
    ...
    private suspend fun getCommitDates(repo: Git): List<LocalDate> = withContext(Dispatchers.IO) {
        ...
        commits.map {
            LocalDate.ofInstant(
                Instant.ofEpochSecond(it.commitTime.toLong()),
                TimeZone.getDefault().toZoneId()
            )
        }
            .toList()
...
data class StudentResults(
    val id: String,
    val taskResults: Map<String, TaskStatus>,
    val taskCommits: Map<String, LocalDate>,
    val commits: List<LocalDate>
)